					...
					<%
					  try {
						  Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/travelplanner","root","Y@du0404");
					    	...
					    	<td><a href="../ModifyPackage?package=<%=rs.getString(1) %>&place=<%=rs.getString(2)%>">Modify</a></td>
					    	<td><a href="../CancelPackage?package=<%=rs.getString(1) %>&place=<%=rs.getString(2)%> ">Cancel</a></td>
					    	</tr>
					    	<%} %>
					    	<% 
					    	}
